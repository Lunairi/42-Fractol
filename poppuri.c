...
void	poppuri_set(t_draw *draw)
{
	...
	draw->f->n_imag = sin(draw->f->o_real
		* draw->f->o_imag) + draw->f->imaginary;
}

void	poppuri_init(t_draw *draw, t_vars v, int h, int w)
{
	draw->f->n_real = 1.5 * (v.x - w / 2)
		/ (0.25 * draw->zoom * w) + draw->f->move_x;
	draw->f->n_imag = (v.y - h / 2)
		/ (0.25 * draw->zoom * h) + draw->f->move_y;
}
