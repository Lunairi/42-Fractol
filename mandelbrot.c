...
void	mandelbrot_set(t_draw *draw)
{
	...
	draw->f->n_imag = 2 * draw->f->o_real
		* draw->f->o_imag + draw->f->imaginary;
}

void	mandelbrot_init(t_draw *draw, t_vars v, int h, int w)
{
	draw->f->real = 1.5 * (v.x - w / 2)
		/ (0.5 * draw->zoom * w) + (draw->f->move_x - 0.55);
	draw->f->imaginary = (v.y - h / 2)
